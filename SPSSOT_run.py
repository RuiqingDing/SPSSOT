...
from models.SPE import SelfPacedSample
...
flags.DEFINE_float('gloss', 0.5, 'the weight of group entropy of target unlabeled data')
...
# convert to one hot encoded vector0.6167
...
source_model.fit(Xs, ys_cat, batch_size=128, epochs=100, validation_data=(Xtl, ytl_cat), verbose = 0)

# Finetune on target labeled data
source_model.fit(Xtl, ytl_cat, batch_size=128, epochs=100, validation_data=(Xtest, ytest_cat), verbose = 0)
...
# SPSSOT model training
spe_model = SelfPacedSample(n_estimators=5,verbose=1).fit(al_model, Xs, ys, Xtl, ytl, Xtu, Xtest, ytest,label_maj=0, label_min=1)

probs_test = spe_model.probs_test
print(np.array(probs_test).shape)
probs_t = np.array(probs_test).mean(axis=0)

print(f'source={FLAGS.source}, target={FLAGS.target}, percent={FLAGS.percent} \not_alpha={FLAGS.ot_alpha}, sloss={FLAGS.sloss}, tloss={FLAGS.tloss}, gloss={FLAGS.gloss}, closs={FLAGS.closs}')
print("\n Test AUC using SPSSOT: ")
